...
class HostView(View):
    ...
    def post(self, request):
        form, error = JsonParser(
            ...
            Argument('group_ids', type=list, handler=lambda x: [int(i) for i in x if i is not None and str(i).strip() != ''], filter=lambda x: len([i for i in x if i is not None and str(i).strip() != '']), help='请选择主机分组'),
